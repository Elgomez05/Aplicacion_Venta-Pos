...
class QueriesSQLServer:
    ...
    def bootstrap_database(db_name):
        try:
            ...
            if exists:
                logging.info(f"DB ya existe: {db_name}")
            else:
                # Crearla si no existe
                cursor.execute(f"CREATE DATABASE [{db_name}]")
                logging.info(f"DB creada correctamente: {db_name}")
            conn.close()
        except Exception as e:
            logging.error(f"Error creando DB desde master: {e}")
            raise
...
if __name__ == "__main__":
    ...
    print(f'modulos cargados correctamente')
    # try:
    #     # 1. Conexión temporal a master SOLO para crear DB
    ...
    #     cursor.execute("""
    #         IF NOT EXISTS (SELECT name FROM sys.databases WHERE name='PuntoventaDB1')
    #         CREATE DATABASE PuntoventaDB1
    #     """)
    #     cursor.close()
    #     temp_conn.close()
    #     logging.info("Base de datos verificada/creada en 'master'")
        
    # except Exception as e:
    #     logging.error(f"Error en creación de DB: {e}")
    
    # 2. Ahora conectar a PuntoventaDB normal
    # try:
    #     connection = QueriesSQLServer.create_connection(server, database, username, password)
        
    #     if connection:
    #         logging.info(f"Conexión exitosa a {database}")
            
    #         # 3. Crear/verificar estructura
    #         QueriesSQLServer.create_tables()
    #         QueriesSQLServer.alter_productos_add_fields()
    #         QueriesSQLServer.alter_usuarios_add_fields()
            
    #         logging.info("Estructura de DB verificada")
            
    # except Exception as e:
    #     logging.error(f"Error conectando a PuntoventaDB: {e}")####v7----
