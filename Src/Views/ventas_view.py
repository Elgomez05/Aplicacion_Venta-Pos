...
from Src.Views.admin_view import SystemManager
...
class VentasWindow(BoxLayout):
    ...
    def __init__(self, actualizar_inventario_callback, terminal_id=1, user_data=None, **kwargs):
        ...
        Clock.schedule_once(self.verificar_bloqueo_sistema, 1)

    def verificar_bloqueo_sistema(self, dt):
        """Verifica y aplica bloqueo del sistema si está desbloqueado"""
        if not self.system_manager.esta_activo():
            self.bloquear_sistema()
        else:
            self.desbloquear_sistema()
    
    def bloquear_sistema(self):
        """Bloquea todos los controles excepto cambiar vista y admin"""
        # Deshabilitar controles de ventas
        controles_bloquear = [
            'buscar_codigo', 'buscar_nombre', 'borrar_articulo',
            'cambiar_cantidad', 'pagar', 'nueva_compra'
        ]
        
        for control_id in controles_bloquear:
            if hasattr(self.ids, control_id):
                self.ids[control_id].disabled = True
                self.ids[control_id].opacity = 0.5
        
        # Mostrar notificación
        self.ids.notificacion_falla.text = "SISTEMA BLOQUEADO - Contacte a su administrador"
        self.ids.notificacion_falla.color = (0.9, 0.2, 0.2, 1)  # Rojo
        
        # El botón de admin sigue disponible para reactivar
        if hasattr(self.ids, 'admin_boton'):
    ...
    def desbloquear_sistema(self):
        ...
        # Habilitar controles de ventas
        controles_desbloquear = [
            'buscar_codigo', 'buscar_nombre', 'borrar_articulo',
            'cambiar_cantidad', 'pagar', 'nueva_compra'
        ]
        
        for control_id in controles_desbloquear:
            if hasattr(self.ids, control_id):
                self.ids[control_id].disabled = False
                self.ids[control_id].opacity = 1
    ...
    def _actualizar_encabezado(self, user_data):
        ...
        tipo_usuario = user_data.get('tipo', 'empleado')
